...
function parseXML(xmlData,cb){
	parseString(xmlData,{trim: true,explicitArray:false},function(err,result){
		if(err){
			console.error('闪豆parseXML数据解析失败:',xmlData,err);
			cb('invalid xml data.');
		}else{
			cb(null,result);
		}
...
Paypalm.prototype.getPayUrl = function(data,cb){
	...
	async.waterfall([
		...
		function(orderid,cb){
			...
			var errMsg = '获取支付加密信息失败，请检查java服务';
			//getWapUrl
			request.post({url:getWapUrl,body:JSON.stringify(payInfo)},function(error, response, body){
				...
				console.log("闪豆获取支付url请求失败:",error,'body:',body,'statusCode:',(util.isObject(response) && response.statusCode),data);
				return cb(errMsg);
			});
		}
	],function(err,payUrl){
...
Paypalm.prototype.getOrderId = function(data,cb){
	if(!util.isObject(data)){
		return cb('the first param must be object.');
	}
	var self = this;
	...
	async.waterfall([
		...
		function(base64Data,cb){
			...
			request.get(getOrderInfoXmlUrl,function(error, response, body){
				...
				var msg = '订单入库失败,error:'+error+',statusCode:'+ (util.isObject(response) && response.statusCode)+',body:'+body+',orderInfo:'+orderInfo;
				console.log(msg);
				return cb(msg);
			})
		},
		...
		if(err){
			console.log('闪豆getOrderId订单入库失败:',err,orderid,orderInfo);
		}
		cb(err,orderid);
	});
}
Paypalm.prototype.paySuccess = function(data,isH5,cb){
	...
	var code = 0,msg = 'ok';
	if(!util.isObject(data)){
		msg = 'pp_flashbean SDK log : the param must be object.';
		console.warn(msg);
		return cb(msg);
	}
	if(!data.transdata){
		msg = '参数不全，需要transdata参数';
		return cb(msg);
	}
	if(isH5){
		url = this.config.GET_H5RETURN_DATA_URL;
	}
	this.parseData(data.transdata,url,function(err,jsonData){
		if(err){
			cb(err,data.transdata);
		}else{
...
Paypalm.prototype.queryWithdraw = function(queryData,cb){
	...
	var code = 1,msg = '代付不成功';
	queryData.productid = this.config.WITHDRAW_PRODUCTID || "LCDF0202";
	self._queryOrder(queryData,function(err,result){
		if(err){
			cb(err,result);
		}else{
			if(result.orderstatus === '1' && result.rspcode === "000000"){
				code = 0 , msg = '代付成功';
			}
			cb(null,{code:code,msg:msg,data:result});
		}	
	});
}
	...
	async.waterfall([
		...
		function(decryptData,cb){
			...
			request.get(queryUrl,function(error, response, body){
				...
				return cb('订单查询失败,error:'+error+',statusCode:'+(util.isObject(response) && response.statusCode)+',body:'+body);
			})
		},
		function(base64Data,cb){
			self._commdata_decrypt(base64Data,cb);
	...
	async.waterfall([
		...
		function(decryptData,cb){
			...
			request.get(queryUrl,function(error, response, body){
				...
				return cb('提现订单入库失败,error:'+error+',statusCode:'+(util.isObject(response) && response.statusCode)+',body:'+body);
			})
		},
		function(base64Data,cb){
			self._commdata_decrypt(base64Data,cb);
...
Paypalm.prototype.withdrawSuccess = function(base64Data,cb){
	var self = this;
	var code = 1,msg = '代付不成功';
	async.waterfall([
		function(cb){
...
Paypalm.prototype._getStopNotifyData = function(cb){
	return this.commStr_encrypt('success',cb);
}
...
Paypalm.prototype._commStr_encrypt = function(text,cb){
	...
	var errMsg = '字符串数据加密失败，请检查java服务';
...
Paypalm.prototype._commdata_encrypt = function(json,cb){
	...
	request.post({url:encodeDataUrl,body:JSON.stringify(json)},function(error, response, body){
		...
		console.log("闪豆_commdata_encrypt数据加密请求失败:",error,'body:',body,'statusCode:',(util.isObject(response) && response.statusCode),'json:',json);
		return cb(errMsg);
	});
}
Paypalm.prototype._commdata_decrypt = function(base64Data,cb){
	...
	request.post({url:decodeDataUrl,body:base64Data},function(error, response, body){
		...
		console.log("闪豆_commdata_decrypt数据解密请求失败:",error,'body:',body,'statusCode:',(util.isObject(response) && response.statusCode),'base64Data:',base64Data);
		return cb(errMsg);
	});
}
