...
function encrypt(publicKey,plaintext,digest){
	var str = '';
	try{
		digest = digest || 'base64';
		var crt = ursa.createPublicKey(getRSAPublicKey(publicKey));
		plaintext = UTF8TGBK.encode(plaintext);
  		str = crt.encrypt(plaintext, 'utf8', digest,ursa.RSA_PKCS1_PADDING);
	}catch(e){
		console.error('rsa encrypt data error : ',e);
	}
	return str;
}
function decrypt(merchantPrivateKey,ciphertext,encode){
	var str = '';
	try{
		...
		str = pem.decrypt(ciphertext, encode, 'utf8', ursa.RSA_PKCS1_PADDING);
		str = UTF8TGBK.decode(str);
	}catch(e){
